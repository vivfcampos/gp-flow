...
# ── Cache de leituras (reduz idas ao banco remoto) ────────────────────────────
def cache_leitura(func):
    """
    Decorador que cacheia o resultado de uma função de LEITURA usando
    st.cache_data. As escritas chamam invalidar_cache() para limpar.
    Fora do Streamlit (testes/scripts), roda sem cache.
    """
    try:
        import streamlit as st
    except Exception:
    ...
    cached = st.cache_data(show_spinner=False)(func)

    def wrapper(*args, **kwargs):
        try:
            return cached(*args, **kwargs)
        except Exception:
            # se o cache falhar por algum motivo, cai para a função direta
            return func(*args, **kwargs)
    wrapper.__name__ = getattr(func, "__name__", "cache_leitura")
    return wrapper


def invalidar_cache():
    """Limpa o cache de leituras após uma escrita, para os dados não ficarem velhos."""
    try:
        import streamlit as st
        st.cache_data.clear()
    except Exception:

...
def abrir_card_kanban(demanda_id: int):
    ...
    novo_impedimento = st.text_input("🚧 Impedimento", value=str(d.get("impedimento") or ""),
                                      key=f"kcard_imp_{demanda_id}")
    nova_obs = st.text_area("📝 Observações", value=str(d.get("observacoes") or ""),
                             height=90, key=f"kcard_obs_{demanda_id}")
    nova_desc = st.text_area("📄 Descrição interna (notas da equipe)",
                              value=str(d.get("descricao_interna") or ""),
...
if modo_daily:
    ...
    for _, d in demandas.iterrows():
        todos.append({"Id": int(d["id"]), "Título": d["titulo"],
                      ...
                      "Impedimento": d.get("impedimento") or ""})
    for _, a in atividades.iterrows():
        todos.append({"Id": int(a["id"]), "Título": a["titulo"],
                      ...
                      "Impedimento": a.get("impedimento") or ""})
...
for estado in ESTADOS_KANBAN:
    ...
    for _, d in demandas.iterrows():
        ...
        items.append({
            ...
            "impedimento": bool(d.get("impedimento")),
    ...
    for _, a in atividades.iterrows():
        ...
        items.append({
            ...
            "impedimento": bool(a.get("impedimento")),
            "horas":       0,
            "score":       "",
        })
